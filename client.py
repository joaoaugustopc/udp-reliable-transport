...
PAYLOAD_SIZE = 1000 
WINDOW = 100         # janela de congestionamento inicialmente fixa
TIMEOUT = 0.2       
...
def run_client(server_host="127.0.0.1", server_port=9000, total_packets=10000):
    ...
    inflight = {}      # seq -> (packet_bytes, send_time) - Pacotes enviados, mas ainda não confirmados. guarda bytes para a retransmissão.

    start = time.time()

    def send_packet(seq: int):
        payload = bytes([seq % 256]) * PAYLOAD_SIZE  # Byte com valor entre 0 a 255 repetido 1000 vezes. Payload com 1000 bytes repetidos. (Só para teste)
    ...
    while send_base < total_packets:
        ...
        if send_base % 1000 == 0 and send_base > 0:
            ...
            print(f"[client] acked={send_base}/{total_packets} inflight={len(inflight)} ~{mbps:.2f} Mbps")
